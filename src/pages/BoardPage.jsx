...
export default function BoardPage() {
  ...
  const handleOpenModal = (status = 'todo') => {
    // 폼 상태를 초기화할 때, 날짜/우선순위 등 일부는 유지하고 나머지는 리셋
    const initialFormState = {
      title: '',
      startDate: form.startDate || new Date().toISOString().slice(0, 10),
      dueDate: form.dueDate || new Date().toISOString().slice(0, 10),
      assignee: currentUser ? String(currentUser.user_id) : '',
      parentTask: '',
      priority: form.priority || 'medium',
      isParentTask: false,
      selectedTags: [],
      status: status, // 여기서 status를 명확하게 설정
    };
  ...
  const handleSubmit = async (e) => {
    ...
    // status가 객체일 경우를 대비하여 문자열 키를 추출
    let statusValue = form.status;
    if (typeof statusValue === 'object' && statusValue !== null) {
      // statusConfig에서 현재 status 객체와 일치하는 키를 찾습니다.
      const statusKey = Object.keys(statusConfig).find(key => statusConfig[key].label === statusValue.label);
      statusValue = statusKey || 'todo'; // 못 찾으면 기본값 'todo'
    }
    ...
    const payload = {
      ...
      status: statusValue ? statusValue : 'todo',
    };
    ...
    // "할 일" 상태 디버깅
    if (safePayload.status === 'todo') {
      console.log('🔍 "할 일" 상태로 업무 생성:', {
        formStatus: form.status,
        payloadStatus: payload.status,
        safePayloadStatus: safePayload.status,
        ...
        <Modal onClose={handleCloseModal}>
          <div className="max-w-2xl mx-auto">
            <h2 className="text-xl font-semibold text-gray-900 mb-6">
              새 업무 추가 {form.status && `(${statusConfig[form.status]?.label})`}
            </h2>

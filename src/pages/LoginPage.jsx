...
function LoginPage({ onLogin }) {
  ...
  const login = useGoogleLogin({
    onSuccess: async tokenResponse => {
      try {
        ...
        // 1. 이메일로 회원 존재 여부 확인
        const checkResponse = await axios.post('/api/v1/check-email', { email });
        
        if (checkResponse.data.exists) {
          // 이미 회원이면 바로 로그인
          const loginResponse = await axios.post('/api/v1/oauth/google', {
  ...
  const handleGoogleSignup = async (e) => {
    ...
    try {
      const response = await axios.post('/api/v1/oauth/google/register', {
        email: googleEmail,
        name: extraName,

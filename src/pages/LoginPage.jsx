  ...
  const handleGoogleSignup = async (e) => {
    ...
    try {
      ...
      if (response.data.message === "회원가입 성공") {
        onLogin(googleEmail, extraPassword);
        navigate('/main', { replace: true });
      }
    } catch (error) {
          ...
          {showExtraForm ? (
            ...
            <>
              ...
              <div className="flex justify-between w-full gap-2 mt-4">
                <div className="w-1/3">
                  <GoogleLoginBtn onLoginSuccess={(email, name) => handleGoogleLoginSuccess(email, name)}/>
                </div>
                <button className="bg-white text-gray-700 border border-gray-300 rounded px-4 py-2 w-1/3 hover:bg-gray-100" onClick={handleKakaoLogin}>
                  Kakao
                </button>
                <button className="bg-green-500 text-white border border-gray-300 rounded px-4 py-2 w-1/3 hover:bg-green-600" onClick={handleNaverLogin}>
                  Naver
                </button>
              </div>
              {/* 회원가입 버튼 */}
